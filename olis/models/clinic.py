...
class clinic_opcaisse(models.Model):
    ...
    state = fields.Selection([('brouillon','Brouillon'),('valide','Validé'),('aannuler','A annuler'),('annule','Annulé'),('regle','Réglé')], string='Etat', size=64, default='brouillon' ,track_visibility='onchange', readonly=True, required=True)
...
class clinic_opcaisse_ligne(models.Model):
    ...
    def calcule_cash_annul_l(self):
        ...
        for box in self:
            ...
            for record in bank_statements[0]:
                ...
                if not record.journal_id.company_id.transfer_account_id:
                    raise UserError(_("Please check that the field 'Transfer Account' is set on the company."))
                idpos_session._create_bank_statement_line(record,self.montant*-1,'Annulation '+self.name,None,box.name)                
    ...
    state = fields.Selection([('brouillon','Brouillon'),('valide','Validé'),('annule','Annulé'),('reculabo','Reçu Labo')], string='Etat', size=64, default='brouillon' ,track_visibility='onchange', readonly=True, required=True)
    ...
    def calcule_cash_annul_d(self):
        ...
        for box in self:
            for record in bank_statements[0]:
                ...
                if not record.journal_id.company_id.transfer_account_id:
                    raise UserError(_("Please check that the field 'Transfer Account' is set on the company."))
                self.idsession._create_bank_statement_line(record,self.montant*-1,'Annulation '+self.name,None,box.name)
    ...
    def calcule_cash(self):
        ...
        for box in self:
            for record in bank_statements[0]:
                ...
                if not record.journal_id.company_id.transfer_account_id:
                    raise UserError(_("Please check that the field 'Transfer Account' is set on the company."))
                box.idrecouvrement._create_bank_statement_line(record,self.totalpaie,self.idpatient.nom,None,box.name)
    ...
    def calcule_cash_annul(self):
        ...
        for box in self:
            for record in bank_statements[0]:
                ...
                if not record.journal_id.company_id.transfer_account_id:
                    raise UserError(_("Please check that the field 'Transfer Account' is set on the company."))
                box.idopcaisse._create_bank_statement_line(record,self.totalpaie*-1,'Annulation '+self.name,None,box.name)
    def revenir(self):
        self.write({'state':'valide'})
        return {
        'context': self.env.context,
